...
public class ExtInfoSniffer {
	// *** chunk of records to read from tables
	static int chunk = 100;

	/**
	 * contains a default list of all binary fields to be searched for
	 * unresolved classes. Format: [tableName]:[binaryFieldName]. This is set by
	 * default in the ExtInfoSnifferView
	 */
	...
	public static HashSet<String> searchAllTablesForUnresolvedClasses(
		...
		for (String sniffer : snifferListArr) {
			...
			if (splitted.length == 2) {
				...
				ExtInfoSnifferView.unresolvedClasses
						.setText(((unresolvedStr.isEmpty()) ? ""
								...
								+ sniffer
								+ " ***");
				ExtInfoSnifferView.unresolvedClasses.getParent().getDisplay()
						.update();
	...
	private static HashSet<String> searchTableForUnresolvedClasses(
		...
		if (counterRes == null) {
			String asdfa = ExtInfoSnifferView.snifferErrorsList.getText();
			System.out.println("ResultSet from query '" + counterSql
					+ "' == null.\r\n    May be there is no such table?");
			String before = "\r\n";
			if (ExtInfoSnifferView.snifferErrorsList.getText().isEmpty())
				before = "";
			ExtInfoSnifferView.snifferErrorsList
					.setText(ExtInfoSnifferView.snifferErrorsList.getText()
							+ before
		...
		while (offset < numOfRecords) {
			...
			for (String tmp : unresolved)
				unresolvedStr = unresolvedStr + tmp + "\r\n";
			ExtInfoSnifferView.unresolvedClasses.setText(((unresolvedStr
					.isEmpty()) ? "" : "*** unresolved classes found ***\r\n")
					+ unresolvedStr
					+ "*** processing "
					+ table
					+ ":"
					+ field
					+ " ***");
			ExtInfoSnifferView.unresolvedClasses.getDisplay().update();
		}
		return ObjectInputStreamSniffer.UNRESOLVEDCLASSES;
	}
